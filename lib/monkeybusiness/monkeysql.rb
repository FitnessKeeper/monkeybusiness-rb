  ...
  class DBClient
    ...
    def import_from_s3(target_class, bucket, key, access_key = MonkeyBusiness::MonkeySQL::Access_Key, secret_key = MonkeyBusiness::MonkeySQL::Secret_Key, queries = MonkeyBusiness::MonkeySQL::Import_Queries)
      begin
        ...
        self.sequel.run(query, :s3_path => s3_path, :credentials => credentials, :delimiter => delimiter, :timeformat => timeformat).all
    ...
    def initialize(connection_string)
      begin
        ...
        @log.debug sprintf("initializing with '%s'", connection_string)

        @sequel = Sequel.connect(connection_string, :loggers => [@log], :sslmode => 'require')
